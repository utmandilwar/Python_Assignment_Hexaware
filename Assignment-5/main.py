...
from exceptions.my_exceptions import EventNotFoundException, InvalidBookingIDException
            ...
            if ch == 1:
                ...
                if p == 1:
                    ...
                    try:
                        ...
                        continue
                    self.open()
                    self.c.execute(f"Select * From eventtable Where event_type = '{et}' ")
                    arr = self.c.fetchall()
                    for i in arr:
                ...
                self.open()
                self.c.execute(f"Select * From bookingtable Where booking_id = {bi}")
                bd = self.c.fetchone()
                self.close()
                try:
                    if bd == None or bd == []:
                ...
                self.c.execute(f"Select event_id From bookingtable Where booking_id = {bi}")
                ei = self.c.fetchone()
                self.close()
                try:
                    if ei == None or ei == []:
                ...
                eid = ei[0]
                self.open()
                self.c.execute(f"Select * From eventtable Where event_id = {eid}")
                eveob = Event(*(self.c.fetchone()))
                self.close()
                self.open()
                self.c.execute(f"Select num_tickets From bookingtable Where booking_id = {bi}")
                nt = self.c.fetchone()[0]
                self.close()
                tbs.cancel_tickets(eveob, nt)
                self.open()
                self.c.execute(f"Delete From bookingtable Where booking_id = {bi}")
                self.mydb.commit()
                self.close()
